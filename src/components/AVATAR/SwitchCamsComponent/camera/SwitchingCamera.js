...
import { /* useDEVICE_ID_Media, */ getVideoDevices } from '../local_hooks/use-deviceId-media';
import { useCardRatio } from "../local_hooks/use-card-ratio";
import { useOffsets } from "../local_hooks/use-offsets";
import { Video, Canvas, Wrapper, Container, Flash, Overlay/* , Button */ } from "./cam_styles";
import styled, { keyframes, css } from "styled-components";
import switchCamImg from '../../../../assets/images/switchCamera.png';
import takeFotoCamImg from '../../../../assets/images/takeFoto.png';
import delFotoCamImg from '../../../../assets/images/trash.png';
//import Spinner from '../../../Spinner/Spinner';
//const CAPTURE_OPTIONS = { audio: false, video: { facingMode: "environment" } }; 
var type = function (obj) {
    return Object.prototype.toString.apply(obj).replace(/\[object (.+)\]/i, '$1').toLowerCase();
};
const SwitchingCamera = ({ isOpen, onCapture, onClear, onError }) => {
    ...
    async function startMediaStream(deviceID) {
        ...
        try {
            ...
            console.error(err); setInError(`GetStream[${mediaIndex - 1}]: ` + err);
            // Handle the error
    ...
    function cleanupMediaStream() {
        try {
            ...
            // Handle the error
            console.error(err); setInError(`cleanupMediaStream[${mediaIndex}]: ` + err);
        }
    };
    /* eslint eqeqeq: 0 */
    const getFacingModePattern = (facingMode) => facingMode == 'environment' ? /rear|back|environment/ig : /front|user|face/ig
    useEffect(() => { GetDevices(); }, []);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => { return () => cleanupMediaStream(); }, []);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => { if (inError.includes('NotReadableError') && isVideoPlaying) setMediaIndex(mediaIndex - 1); onError(inError); }, [inError]);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => { isOpen(isVideoPlaying); }, [isVideoPlaying]);

    useLayoutEffect(() => {
        if (inputDevices && inputDevices.length > 0) {
            const pattern = getFacingModePattern('user');

            const filteredDevices = inputDevices.filter(({ label }) => pattern.test(label)); // Filter out video devices without the pattern
            if (filteredDevices && filteredDevices.length > 0) {
                if (filteredDevices.length > 1) { setPlayingDeviceID(filteredDevices[1].deviceId); setMediaIndex(1); }
                else { setPlayingDeviceID(filteredDevices[0].deviceId); setMediaIndex(0); }
            } else { const facing = 'user'; if (inputDevices.length == 1 || facing == 'user') { setPlayingDeviceID(inputDevices[0].deviceId); setMediaIndex(0); } else { setPlayingDeviceID(inputDevices[1].deviceId); setMediaIndex(1); } }
            //  return cleanupMediaStream();
        }
        else setInError('no video input devices available');
    }, [inputDevices]);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useLayoutEffect(() => { if (playingDeviceID) { const index = (inputDevices.map(dev => dev.deviceId)).indexOf(playingDeviceID); setMediaIndex(index); startMediaStream(playingDeviceID); } }, [playingDeviceID]);
    useLayoutEffect(() => {
                ...
                <div style={{ display: 'flex', flexDirection: 'row', width: '100%', justifyContent: 'space-around', alignItems: 'center', marginTop: '5%', marginBottom: '5%' }}>
                    <div onClick={() => { setIsFlashingSwitch(!isFlashingSwitch); switchCamera() }} style={{ display: 'flex', flex: 0.75, height: '80%', alignItems: 'flex-start' }}>
                        <FlashingDiv flash={isFlashingSwitch} onAnimationEnd={() => setIsFlashingSwitch(false)}>
                            <img src={switchCamImg} alt='switch' style={{ maxWidth: '30px', /*  height: '100%', */ resize: 'contain', alignSelf: 'center' }} />
                        </FlashingDiv>
                    </div>
                    <div onClick={() => { setIsFlashingTakePhoto(!isFlashingTakePhoto); handleCapture() }} style={{ display: 'flex', flex: 1.5, justifyContent: 'center', alignItems: 'center' }}>
                        <FlashingDiv flash={isFlashingTakePhoto} onAnimationEnd={() => setIsFlashingTakePhoto(false)}>
                            {isVideoPlaying && (
                                <img src={takeFotoCamImg} alt='take' style={{ maxWidth: '50px',  /*  height: '100%', */ resize: 'contain', alignSelf: 'center' }} />
                    ...
                    <div onClick={() => { setIsFlashingDelete(!isFlashingDelete); handleClear(); }/* handleClear */} style={{ display: 'flex', flex: 0.75, height: '80%', alignItems: 'flex-end', justifyContent: 'flex-end' }}>
                        <FlashingDiv flash={isFlashingDelete} onAnimationEnd={() => setIsFlashingDelete(false)}>
                            {isVideoPlaying && (
                                <img src={delFotoCamImg} alt='del' style={{ maxWidth: '30px', /*  height: '100%', */ resize: 'contain', alignSelf: 'center' }} />
                            )}
                        </FlashingDiv>
                    </div>
                </div>
