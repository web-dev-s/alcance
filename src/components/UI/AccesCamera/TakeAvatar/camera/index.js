...
export function Camera({ onCapture, onClear, facingString }) {
  ...
  const mediaStream = useUserMedia(CAPTURE_OPTIONS, facingMode);
  ...
  if (mediaStream && videoRef.current && !videoRef.current.srcObject) { videoRef.current.srcObject = mediaStream; }
  ...
  function handleClear() {
    const context = canvasRef.current.getContext("2d"); context.clearRect(0, 0, canvasRef.current.width, canvasRef.current.height); setIsCanvasEmpty(true); onClear();
  }
  ...
  const SwitchCamera = async () => { setFacingMode(!facingMode); }
  const processDevices = (devices) => { devices.forEach(device => { console.log(device.label);     /*  this.setDevice(device);  */ }); setFacingMode(!facingMode); }
  return (
...
//export default React.forwardRef((props, ref)=>(<Camera/>));
